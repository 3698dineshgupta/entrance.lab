    ...
    <section className="mx-auto w-full max-w-7xl px-4 py-8 md:px-6">
      <div className="relative md:min-h-[560px] overflow-hidden rounded-[20px] border border-white/10 bg-black shadow-2xl">
        ...
        <div className="relative z-10 flex flex-row md:grid md:grid-cols-[48%_52%] md:min-h-[560px] items-center">
          {/* Left: content */}
          <div className="flex flex-col justify-center w-[55%] md:w-full px-4 sm:px-6 md:px-12 lg:px-16 py-6 md:py-12 z-20">
            ...
            <div className="mt-5 md:mt-8 flex flex-row gap-2">
              <button
                type="button"
                onClick={() => setOpen(true)}
                className="rounded-lg md:rounded-xl bg-blue-600 px-3 py-2 md:px-6 md:py-3.5 text-[10px] md:text-base font-semibold text-white transition hover:bg-blue-500 whitespace-nowrap"
              >
                Select Exam
              </button>
              <Link
                href="/mock-tests"
                className="inline-flex items-center justify-center rounded-lg md:rounded-xl border border-white/20 bg-white/5 px-3 py-2 md:px-6 md:py-3.5 text-[10px] md:text-base font-semibold text-white transition hover:bg-white/10 whitespace-nowrap"
              >
                Mock Test
              </Link>
            </div>

            <div className="mt-4 md:mt-8 flex flex-wrap items-center gap-2 md:gap-5 text-[9px] md:text-xs text-neutral-400">
